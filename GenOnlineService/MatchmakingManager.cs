						...
						else
						{
							...
							if (m_dictMatchmakingBuckets.ContainsKey(thisSession.MatchmakingPlaylistID))
							{
								...
								foreach (MatchmakingBucket mmBucket in m_dictMatchmakingBuckets[thisSession.MatchmakingPlaylistID])
								{
									...
									int eloExpansionToUse = (mmBucket.GetAvgElo() >= EloConfig.HighEloThreshold || thisSessionStats.EloRating >= EloConfig.HighEloThreshold) ? EloConfig.EloExpansionValue_HighELO : EloConfig.EloExpansionValue_Standard;
									if (mmBucket.IsAvgEloWithinThreshold(thisSessionStats.EloRating, eloExpansionToUse))
									{
