		...
		public async Task<APIResult?> Monitor_CheckLogin()
		{
			...
			else
			{
				try
				{
					...
					APIResult internalResult = await checkLoginController.Post_InternalHandler("{\"challenge\": \"abc\", \"nonce\": \"def\", \"code\": \"iamatest\", \"client_id\": \"gen_online_30hz\"}", IPAddress.Loopback.ToString(), Program.GetWebSocketAddress(Request));
					return internalResult;
				}
				catch
				{
