...
namespace GenOnlineService
{
	...
	static class WebSocketManager
	{
		...
		*/
		...
		public static async Task<bool> ClearDataFromUser(Int64 userID, EUserSessionType sessionType)
		{
			...
			{
				itemRemoved.ReleasePendingWebsocketSendAccounting();
			}
			return removed;
		}
	...
	public class UserSession
	{
		...
		// Called when the session is removed and will never be ticked again; keeps the send-queue gauge accurate without altering the queue.
		public void ReleasePendingWebsocketSendAccounting()
		{
			int pending = m_lstPendingWebsocketSends.Count;
			if (pending > 0)
			{
				AppMetrics.AdjustWebSocketSendQueueDepth(-pending);
			}
		}
